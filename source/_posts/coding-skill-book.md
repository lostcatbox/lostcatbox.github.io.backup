...
상하좌우/대각선으로 인접한 칸들의 글자들을 이어서 단어를 찾아내는것, 대각선으로도 가능하며, 한글자가 두번 이상 사용될수도있다. 주어진 칸에서 시작해서 특정 단어를 찾을수 있는지 확인하는 문제를 풀자 

![스크린샷 2020-11-19 오후 8.42.58](https://tva1.sinaimg.cn/large/0081Kckwgy1gkupx6zkngj31840eynot.jpg)

> (처음접한 생각)
>
> 필요한 것은 x,y 좌표와 word가 필요하다.
>
> 완전탐색으로 다 찾아버리기(경우의 수 다 따져야함)
>
> 아마 일을 조각을 내서, 재귀 함수로 구현하는게 좋지 않을까?

주의할점은 다음 글자가 될수있는게 여러가지 있을때, 그당시에 뭘 타면 성공하고, 뭘타면 실패할지 알수없다. 완전탐색을 통해 해결가능

### 문제의 분할

`hasWord(y,x,word)`

이 함수에서 하는 일을 조각내는 방법은 각 글자를 하나의 조각으로 만드는 것이다. 처음 위치에서 word[0]글자와 일치하지 않는다면, 그 즉시 `False` 를 반환하고 종료하면된다. 만약 첫글자가 맞는다면, 원래단어에서 첫글자를 빼고 이를 격자에서 찾아야한다. 그 시작위치(x,y)는 당연히 word[0]가 맞은 (x,y)에 인접한 8개 칸중에 하나일것이다. 따라서 8경우를 모두 시도하여 답찾으면된다.

### 기저 사례의 선택

더 이상 탐색 없이 간단히 답을 낼수있는 다음 경우들을 기저 사례로 선택한다.

(글자를 찾으면 빼나보네.. 그래서 진행할때마다 word첫글자가 바뀜)(확인완료)

1. 위치(x,y)에 있는 글자가 원하는 단어의 첫글자가 아닌경우

2. (1번 경우에 해당되지 않을 경우) 원하는 단어가 한글자인 경우 항상 성공

   (위에서 실패안했으니까 한글자가 맞는거지!)

순서도 중요!

__간결한 코드 작성에 유용한 팁은, 입력이 잘못되거나 범위에서 벗어난 경우도 기저 사례로 택해서 맨 처음에 처리하는것이다__

맨처음 처리해줘야 반복코드가실행안됨. 따라서 위 1,2 뿐만아니라 현재 위치가 게임판을 벗어난경우와, 첫 글자가 일치하지 않는 경우를 모두 포함해서 기저 사례로 선택해서 처리하자

### 구현

hasWord()의 처음에서 기저 사례를 처리하므로서 for문안에서 벼도로 확인하지 않아도된다는점이 유리하다. 다음 칸의 상대 좌표 목록을 함수내에 직접 코딩해서 넣은것이아닌, 별도의 변수로 분리해 낸점도 포인트

```python
def inRange(y, x):
    if y in range(5) and x in range(5):
        return True
    return False
...
## 5x5의 보글 게임판의 해당 위치에서 주어진 단어가 시작하는 지를 반환
def hasWord(y, x, word):
    ...
    for direction in range(8):
        ...
        ## 다음칸이 범위안에 있는지, 첫 글자가 일치하는지 확인할 필요없다
        if hasWord(nextY, nextX, word[1:]):
            return True
    return False


board = [["U", "R", "L", "P", "M"], ["X", "P", "R", "E", "T"], ["G", "I", "A", "E", "T"], ["X", "T", "N", "Z", "Y"],
         ["X", "O", "Q", "R", "S"]]
...
완전 탐색 알고리즘의 계산은 단순하다. 가능한 후보의 수를 전부 세어보기만 하면되니까. 8칸을 보니까 최대 검사는 8^(n-1)이다.

O(8^N)

지수적 증가하므로 단어의 길리가 길어질경우 다른 설계 패러다임을 사용

### 완전 탐색 레시피

완전탐색으로 생각된다면 처음으로 어떤식으로 접근해야하나?

1. 완전 탐색은 존재하는 모든 답을 하나씩 검사하므로, 걸리는 시간은 가능한 답의 수와 정확히 비례한다. 
2. 가능한 모든 답의 후보를 만드는 과정을 여러개의 선택으로 나눈다. 각 선택은 답의 후보를 만드는 과정의 한 조강이 된다
3. 그중 하나의 조삭을 선택해 답의 일부를 만들고, 나머지 답을 재귀 호출을 통해 완성한다
4. 조각이 하나밖에 남지 않은 경우, 혹은 하나도 남지 않은 경우에는 답을 생성했으므로, 이것을 기저 사례로 선택해 처리합니다.

첫번째는 현재 위치(y,x)에 단어의 첫 글자가 있는가

두번째부터는 윗칸(y-1,x)에서 시작해서, 단어의 나머지 글자들을 찾을수있는가?

세번째부터 두번째반복..위치만 바꿔서

두번째 부터는 문제를 구성하는 조각을 떄어냈기때문에 이를 부분 문제라고함.

# 순서대로 읽기는 중단...

시간이 너무 오래걸린다. 실전을 통해 배우기위해 프로그래머스 1~2단계부터 차근차근 문제를 풀어갈것이고 해당 개념이 처음보는것들이거나 자료 구조일경우 책으로 돌아와 정리할것이다
