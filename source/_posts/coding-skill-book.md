...
7장에서 다루는 분할 정복 기법을 이용하면 더 빠른 수행시간 가지는 알고리즘 설계가 가능하다. 입력받은 배열을 우선 절반으로 잘라 왼쪽 배열과 오른쪽 배열로 나눈다. 이때 우리가 원하는 최대 합 부분 구간은 두 배열 중 하나에 속해있을수도있고, 두 배열 사이에 걸처있을수도있다. 이떄 각 경우의 답을 재귀 호출과 탐욕법을 이용해 계산하면 훌륭한 분할 정복 알고리즘이된다. 아래는 전체 수행 시간은 O(NlogN)
