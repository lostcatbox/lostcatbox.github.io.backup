...
__이러한 코드에서 필요한 것은 상호 배제이다__. 즉, 이 속성은 하나의 쓰레드가 임계 영역 내의 코드를 실행 중일 때는 다른 쓰레드가 실행할 수없도록 보장해준다.

__원자성!__ 전부 아니면 전무이다. 원자성을 보장한다는 것은명령어가 실행이 안되었거나 실행이 종료된 후라는것.
